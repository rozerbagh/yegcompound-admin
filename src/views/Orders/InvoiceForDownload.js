                ...
                <Page size="A4" style={styles.page}>
                    <View style={styles.invoiceFirst}>
                        <View>
                            ...
                            <Text style={styles.info}>
                                Status :{' '}
                                {data.status === 'pending' || data.status === 'ordered' && (
                    ...
                    <View style={styles.invoiceSecond}>
                        ...
                        <View>
                            ...
                            <Text style={styles.info}>{data.user.fullname},{data.user.email}, {data.user.phoneno}</Text>
                            <Text style={styles.info}>
                                {data.user.address[0]?.street},{data.user.address[0]?.landmark},
                            </Text>
                            <Text style={styles.info}>
                                {data.user.address[0]?.city}, {data.user.address[0]?.country}, {data.user.address[0]?.pincode}
                            </Text>
                        </View>
                    </View>
                        ...
                        {data?.ingredients?.map((item, i) => (
                            <View key={i} style={styles.tableRow}>
                                ...
                                <View style={styles.tableCol}>
                                    <Text style={styles.tableCell}>
                                        {' '}
                                        <span style={styles.quantity}>&#8377;{item.pack_size}</span>{' '}
                                    </Text>
                                </View>
                    ...
                    <View style={styles.invoiceThird}>
                        ...
                        <View>
                            ...
                            <Text style={styles.info}> &#8377; {Math.round(data.total_price).toFixed(2)}</Text>
                        </View>

                        <View>
                            <Text style={styles.title}>Need To Pay</Text>
                            <Text style={styles.amount}>&#8377; {Math.round(data.need_to_pay).toFixed(2)}</Text>
                        </View>
                    </View>
                </Page>
            </Document>
