...
const OrdersDetails = () => {
    ...
    const handleUpdateStatus = (status) => {
        api.patch("/app/order/update/" + orderid, { status: status }).then(({ data }) => {
            console.log(data);
            alert(data.message)
        }).catch(err => {
                                ...
                                <Card className="card-profile shadow" ref={reportTemplateRef}>
                                    ...
                                    <CardBody className="pt-0 mt-5">
                                        <div className="text-center">
                                            ...
                                            <div>
                                                Order Status &nbsp;
                                                <UpdateStatus allStatus={orderStatus}
                                                    status={ordersDetails.status}
                                                    handleStatus={(status) => { handleUpdateStatus(status) }}
                                                />
                                            </div>
                                            <div>
                                                <i className="ni education_hat mr-2" />
                                                {ordersDetails.user.email},{ordersDetails.user.phoneno}
                                            </div>
                                                ...
                                                <Col xs={12}>
                                                    <Table>
                                                        ...
                                                        <tbody>
                                                            ...
                                                            <tr>
                                                                <td colSpan={3}></td>
                                                                <td>Total Price - {(ordersDetails.total_price).toFixed(2)}</td>
                                                                <td>Pay - {(ordersDetails.need_to_pay).toFixed(2)}</td>
                                                            </tr>
                                                        </tbody>
                                                    </Table>
                                                </Col>
